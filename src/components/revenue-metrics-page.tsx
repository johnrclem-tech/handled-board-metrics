...
export function RevenueMetricsPage({ segment }: RevenueMetricsPageProps) {
  ...
  // Compute 100% stacked mix data (with yoyTotal for growth line)
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle className="flex items-center gap-2">
            Revenue by Service
            <InfoTooltip text="Stacked revenue by service type with total year-over-year growth rate. Toggle between dollar amounts and percentage mix." />
          </CardTitle>
            ...
            <ResponsiveContainer width="100%" height={350}>
              <ComposedChart data={dataset} margin={{ top: 15, right: 20, left: 15, bottom: 5 }}>
                ...
                <YAxis
                  ...
                  tick={{ fontSize: 11 }}
                  width={55}
                  label={{ value: "Revenue ($)", angle: -90, position: "center", dx: -20, style: { fontSize: 13, fontWeight: 600, fill: "hsl(var(--muted-foreground))" } }}
                />
                <YAxis
                  yAxisId="right"
                  orientation="right"
                  ...
                  tick={{ fontSize: 11 }}
                  width={55}
                  label={{ value: "Growth (%)", angle: 90, position: "center", dx: 20, style: { fontSize: 13, fontWeight: 600, fill: "hsl(var(--muted-foreground))" } }}
                />
                <Legend />
